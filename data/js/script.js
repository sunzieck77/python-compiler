...
function reSizeOutput() {
    var outputHead = document.getElementById('output-head');
    var outputTextarea = document.getElementById('output');

    if (!outputHead || !outputTextarea) {
        console.error("Output head or output textarea not found.");
        return;
    }

    outputHead.addEventListener('mousedown', function (event) {
        if (event.buttons === 1) { // Left mouse button clicked
            // Start resizing
            var startY = event.clientY;
            var startHeight = parseInt(getComputedStyle(outputTextarea).height);
...
function resPanel() {
    ...
    var consoleBtn = document.querySelector("console-btn");
